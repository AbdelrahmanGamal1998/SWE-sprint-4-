...
/*
 * public void undoEdit(Products product) throws IOException { File file = new
 * File("Before Edit.txt"); FileReader FREADER = new FileReader(file); Scanner
 * read = new Scanner(file); String Name_line = null, Category_line = null;
 * String Price_line = null, Brand_line = null; while (read.hasNext()) {
 * Name_line = read.nextLine(); Price_line = read.nextLine(); Brand_line =
 * read.nextLine(); Category_line = read.nextLine();
 * 
 * if (product.getName().contentEquals(Name_line)) { product.setName(Name_line);
 * product.setPrice(Price_line); product.setBrand(Brand_line);
 * product.setCategory(Category_line); System.out.println(product.getName());
 * System.out.println(product.getPrice());
 * System.out.println(product.getBrand());
 * System.out.println(product.getCategory()); break; }
 * 
 * } for (int i=0;i<products.size();i++) { if
 * (product.getName().contentEquals(products.get(i).getName())) {
 ...
 * }
 * 
 * PrintWriter writer = new PrintWriter("StoreProducts.txt"); writer.print("");
 * writer.close();
 * 
 * File file1 = new File("StoreProducts.txt");
 * 
 * /*
 * 
 * for (int j = 0; j < products.size(); j++) { Name_line=
 * products.get(j).getName(); Price_line=products.get(j).getPrice();
 * Brand_line=products.get(j).getBrand();
 * Category_line=products.get(j).getCategory(); /*
